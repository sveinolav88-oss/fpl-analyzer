...
def _starting_xi(squad):
    groups = {
        p: sorted(
            ...
            key=lambda x: float(x.get("expected_gw_points", 0))
            + .12 * float(x.get("minutes_probability", .7)),
            reverse=True,
        )
        for p in REQUIRED
    }
...
def _objective(squad):
    """Actual squad objective: maximize expected GW1 points, with bench value as a secondary factor."""
...
def _improve_by_swaps(squad, pools, budget_units, locked_ids):
    """Local search after beam construction.

    The beam is deliberately bounded for Streamlit performance. This second pass
    checks materially better replacements from every position, which prevents a
    good-looking but unnecessarily cheap beam result from being treated as global optimum.
    """
