...
.player-card{text-align:center;min-height:112px;padding:.15rem .1rem .25rem}.player-avatar-wrap{position:relative;width:72px;height:72px;margin:0 auto}.player-card img,.player-avatar-fallback{width:72px;height:72px;object-fit:contain;border-radius:50%;border:3px solid rgba(255,255,255,.9);background:#20242d;display:block;margin:0 auto}.player-avatar-fallback{display:none;align-items:center;justify-content:center;color:white;font-weight:850}.player-card .name{font-weight:850;font-size:.88rem;margin-top:.18rem;color:white;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.player-card .team{color:rgba(255,255,255,.72);font-size:.72rem}.player-card .meta{color:rgba(255,255,255,.82);font-size:.69rem}.locked-badge{display:inline-block;margin-top:.12rem;padding:.1rem .38rem;border-radius:999px;font-size:.62rem;background:rgba(239,94,86,.20);color:#ffd1cc}
.bench-card{padding:.65rem .45rem;border-radius:14px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.045);text-align:center}.bench-avatar-wrap{position:relative;width:58px;height:58px;margin:0 auto}.bench-card img,.bench-avatar-fallback{width:58px;height:58px;object-fit:contain;border-radius:50%;border:2px solid rgba(255,255,255,.7);display:block;margin:0 auto;background:#20242d}.bench-avatar-fallback{display:none;align-items:center;justify-content:center;color:white;font-weight:800}.bench-card .name{font-weight:750;font-size:.82rem;margin-top:.2rem}.bench-card .meta{font-size:.7rem;opacity:.68}.captain-card{padding:1rem;border-radius:16px;border:1px solid rgba(255,255,255,.08);background:rgba(255,255,255,.025);text-align:center;min-height:250px}.captain-title{font-size:.72rem;font-weight:850;letter-spacing:.12em;opacity:.65;margin-bottom:.5rem}
...
def image_tag(p,size=72,bench=False):
    ...
    wrap="bench-avatar-wrap" if bench else "player-avatar-wrap"
    if url:
        fallback=esc(initials(p))
        return f'<div class="{wrap}"><img src="{esc(url)}" width="{size}" height="{size}" loading="lazy" alt="{esc(p.get("name",""))}" onerror=\'this.style.display="none";this.nextElementSibling.style.display="flex"\'><span class="{cls}">{fallback}</span></div>'
    return f'<div class="{wrap}"><span class="{cls}" style="display:flex">{esc(initials(p))}</span></div>'
